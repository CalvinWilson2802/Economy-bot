...
client.on("message", async message => {
    ...
    if (message.content == "<@!822068130989342731>") {
        return message.lineReply(`Hi, ${message.author.username} My prefix is \`.help\``);
    }
    if (message.content == "<@822068130989342731>") {
        return message.lineReply(`Hi, ${message.author.username} My prefix is \`.help\``);
    }
    ...
    if (mention) {
        try {
            ...
            if (user2.afk == 1 && message.guild.id == user2.afkguildid) {
                message.lineReply(`**${mention.username}** is **AFK** **${user2.afkdate}**\nMessage: ${user2.afkmessage}`);
            }
        } catch (err) {}
    }

    let prefix = "." || message.content == "<@!822068130989342731>";
...
`)
	...
	.setFooter(`Requested by ${message.author.tag}`, message.author.displayAvatarURL({ dynamic: true }))
        ...
        try {
            ...
            if (!fs.existsSync("./newusers/" + message.author.id + ".json")) {
                ...
                user2.ances
                            ...
                            .setTimestamp();
                        return message.lineReply(embeds);
        ...
        try {
            if (message.author.id == "780615402312433674") {
                console.log("galvin"); //850333458660327445
            } else if (message.author.id == "549920365279707147") {
                console.log(`cyrus`);
            } else {
                return message.lineReply(":clown: you dont have permission to do that");
            }
            const mention = getUserFromMention(args[0]);
            if (!fs.existsSync("./newusers/" + mention + ".json")) {
                ...
                fs.writeFile(user, JSON.stringify(user2), function writeJSON(err) {
                    if (err) {
                        ...
                        message.lineReply(`successfully reseted ${mention}'s gems!`);
                    }
                });
            }
        } catch (err) {
        ...
        if (mention == message.author.id) {
            return message.lineReply(`You can't do that.`);
        }
        ...
        if (!fs.existsSync("./newusers/" + mention + ".json")) {
            ...
            let user2 = require(user);
            if (user2.ban == 1) {
                return message.lineReply(`**(${mention.tag})** already blacklisted.`);
            }
            user2.ban = 1;
            ...
            fs.writeFile(user, JSON.stringify(user2), function writeJSON(err) {
              ...
              .join(" ")}!`
                  ...
                  .join(" ")})**`
                            ...
                            .catch(err =>
                                message.lineReply(`Unable to contact the user.`)
                            );
                    } catch (err) {
                        return;
                    }
        ...
        if (!fs.existsSync("./newusers/" + mention + ".json")) {
            ...
            let user2 = require(user);
            if (user2.ban == 0) {
                return message.lineReply(`**(${mention.tag})** is not blacklisted.`);
            }
            user2.ban = 0;
            user2.wrong = 0;
            fs.writeFile(user, JSON.stringify(user2), function writeJSON(err) {
                if (err) {
                    ...
                    message.lineReply(`successfully removed ${mention} blacklist!`);
                    mention
                        .send(
                        ...
                        .catch(err => message.lineReply(`Unable to contact the user.`));
                }
            });
        }
        ...
        if (!fs.existsSync("./newusers/" + mention + ".json")) {
            ...
            if (user2.ban == 1) {
                ...
                message.lineReply(`There is no problem from that user so far.`);
            }
        }
        ...
        if (Number.isNaN(+args[1]) == true) {
            return message.lineReply("Please use the correct syntax.");
        ...
        try {
            ...
            if (Number.isNaN(+args[1]) == true) {
                ...
                return message.lineReply(`cannot find the user or the user is not registered yet.`);
            } else {
        ...
        if (!args[0]) {
            return message.lineReply(`please mentions the user`);
        }
        const mention = getUserFromMention(args[0]);
        if (!fs.existsSync("./newusers/" + mention + ".json")) {
            return message.lineReply(
        ...
        if (!fs.existsSync("./newusers/" + mention + ".json")) {
            ...
            fs.writeFile(user, JSON.stringify(user2), function writeJSON(err) {
                if (err) {
                    ...
                    message.lineReply(`successfully reseted **__${mention}__** id!`);
                }
            });
        }
    } else if (command == "pay") {
        try {
            if (args[1] < 50) {
                return message.lineReply(`Your payment must be more than 50.`);
            }
            const mention = getUserFromMention(args[0]);
            ...
            if (clientuser2.playid == 1) {
                return message.lineReply("you cannot make any transaction for now");
            }
            if (mention.id == message.author.id) {
                return message.lineReply("you can't do that");
            } else if (parseInt(nominal) > parseInt(clientuser2.money)) {
                return message.lineReply("you don't have enough money.");
            } else if (message.content.includes("-")) {
                return message.lineReply("you cannot send negative amount.");
            } else {
                user2.money = parseInt(user2.money) + parseInt(args[1]);
        ...
        try {
            ...
            if (parseInt(nominal) > parseInt(user2.money)) {
                return message.lineReply("you don't have enough money to deposit.");
            } else if (message.content.includes("-")) {
                return message.lineReply("you cannot send negative amount.");
            } else {
        ...
        try {
            ...
            if (parseInt(nominal) > parseInt(user2.deposit)) {
                return message.lineReply("you don't have enough money to withdraw.");
            } else if (message.content.includes("-")) {
                return message.lineReply("you cannot send negative amount.");
            } else {
                user2.money = parseInt(user2.money) + parseInt(args[0]);
        ...
        try {
            ...
            if (!args[0] && user2.capcha == 1) {
                return message.lineReply("Please put the answer.");
            }
                ...
                if (args[0] == user2.code && user2.capcha == 1) {
                    ...
                    fs.writeFile(user, JSON.stringify(user2), function writeJSON(err) {
                        if (err) {
                            ...
                            return message.lineReply("Successfully finished captcha.");
                        }
                    });
                } else {
                    if (user2.capcha == 1) {
        ...
        try {
            ...
            if (user2.ban == 1) {
                ...
                if (user2.farmable < 1) {
                    return message.lineReply(`You don't have enough farmable, please purchase it from store. Type **${prefix}buy farmable**.`);
                }
                ...
                fs.writeFile(user, JSON.stringify(user2), function writeJSON(err) {
                    if (err) {
                        ...
                        return message.lineReply(`**${message.author.username}**, [Ances] You found ⏣ **${ancesgems}** **(${user2.farmable})**🌱 Farmables left.`);
                    }
                });
                ...
                fs.writeFile(user, JSON.stringify(user2), function writeJSON(err) {
                    if (err) {
                        ...
                        return message.lineReply(`**${message.author.username}**, You found ⏣ **${normalgems}** **(${user2.farmable})**🌱 Farmables left.`);
                        limitation8.add(message.author.id);
                        setTimeout(() => {
                            limitation8.delete(message.author.id);
                        }, 2500);
        ...
        try {
            ...
            if (reminder == "pickaxe") {
                if (user2.pickaxe == 1) {
                    return message.lineReply("you already have pickaxe");
            ...
            } else if (reminder == "ances") {
                if (user2.ances == 1) {
                    return message.lineReply("you already have ances");
        ...
        if (!afkmain) {
            try {
                ...
                fs.writeFile(main, JSON.stringify(main2), function writeJSON(err) {
                    if (err) {
                        return message.lineReply("fuck the error");
                    } else {
                        message.lineReply("i set your AFK.");
            ...
            try {
                ...
                fs.writeFile(main, JSON.stringify(main2), function writeJSON(err) {
                    if (err) {
                        ...
                        message.lineReply(`i set your AFK: ${afkmain}.`);
                        message.member.setNickname(`#[AFK] ${message.author.username}`).catch(err => console.log(err))
                    }
                });
            } catch (err) {}
        ...
        try {
            ...
            if (amount < 1) {
                return message.lineReply(`Your bet must be more than 1.`);
            }
            let user = `./newusers/` + message.author.id + ".json";
            let user2 = require(user);
            if (user2.ban == 1) {
            ...
            if (user2.playid == 1) {
                return message.lineReply("you are playing another game");
            }
            if (user2.money == 0) {
                return message.lineReply("Your atleast need 1 gems?");
            } else if (parseInt(amount) > parseInt(user2.money)) {
                return message.lineReply("you don't have enough money.");
            } else if (message.content.includes("-")) {
                return message.lineReply("you cannot bet negative amount.");
            } else if (args[0] > 200000) {
                return message.lineReply("bet can't be more than 200,000");
            } else if (args[0] == "max") {
                ...
                return message.lineReply(`You need to bet something more than ⏣ 100.`)
            ...
            if (int1 == 0 && int2 == 0) {
              ...
              client.user.username
                    ...
                    .setColor("#000000")
                    .setTimestamp();
                return message.lineReply(embed); //this is super duper rare casee....
            } else if (int1 == 0) {
                user2.money = parseInt(user2.money) + parseInt(args[0]);
                fs.writeFile(user, JSON.stringify(user2), function writeJSON(err) {
                    if (err) {
              ...
              client.user.username
                    ...
                    .setColor("#000000")
                    .setTimestamp();
                return message.lineReply(embed5); //this is super duper rare casee....
            } else {
                user2.money = parseInt(user2.money) + parseInt(args[0]);
                fs.writeFile(user, JSON.stringify(user2), function writeJSON(err) {
                    if (err) {
        ...
        try {
            ...
            if (int1 == 0 && int2 == 0) {
              ...
              client.user.username
                            ...
                            .setColor("#FF0000")
                            .setTimestamp();
                        return message.lineReply(embed9)
                    }
                });
            } else if (parseInt(qq(int1)) == parseInt(qq(int2))) {
              ...
              client.user.username
                    ...
                    .setColor("#000000");
                return message.lineReply(embed10);
            } else {
                user2.money = parseInt(user2.money) + parseInt(args[0]);
                fs.writeFile(user, JSON.stringify(user2), function writeJSON(err) {
                    if (err) {
              ...
              client.user.username
                            ...
                            .setColor("#00FF00")
                            .setTimestamp();
                        return message.lineReply(embed11)
                    }
                });
            }
        ...
        return message.lineReply("Disabled due Disk Overloading.");
        ...
        try {
            ...
            if (user2.playid == 1) {
                ...
                return message.lineReply(`You must drop more than 100.`);
            } else if (user2.drop == 1) {
                return message.lineReply(
                    "You must wait untill someone collect your wallet."
                );
            } else if (args[0] < 100) {
                return message.lineReply(`You can't drop less than 1000.`);
            } else if (args[0] > 200000) {
                return message.lineReply("You can't drop more than 200000");
            } else if (Number.isNaN(+args[0]) == true) {
                return message.lineReply(
                    "are you dumb? you must put numbers you want to drop away."
                );
            } else if (user2.money < args[0]) {
                return message.lineReply("You don't have enough gems to drop away.");
            } else {
                user2.money = parseInt(user2.money) - parseInt(args[0]);
        ...
        try {
            ...
            if (user2.playid == 1) {
                ...
                if (user2.money == 0) {
                    ...
                    return message.lineReply("Please put amount do you want to pickup.");
                } else if (Number.isNaN(+args[0]) == true) {
                    return message.lineReply(
                        "are you dumb? you must put numbers you want to drop away."
                    );
        ...
        return message.lineReply(`💫 ${message.author.username}, This command is disabled because it causes the infinite bug is also almost unusable.`);
    } else if (command == "beg") {
        try {
            var gems = beggems();
            ...
            if (user2.ban == 1) {
                ...
                fs.writeFile(user, JSON.stringify(user2), function writeJSON(err) {
                    if (err) {
                        ...
                        return message.lineReply(embed12);
                    }
                });
            } else if (bruh == 13) {
                ...
                fs.writeFile(user, JSON.stringify(user2), function writeJSON(err) {
                    if (err) {
                        ...
                        return message.lineReply(embed13);
                    }
                });
            } else if (bruh == 14) {
                ...
                fs.writeFile(user, JSON.stringify(user2), function writeJSON(err) {
                    if (err) {
                        ...
                        return message.lineReply(embed15);
                    }
                });
            } else if (bruh == 16) {
                ...
                fs.writeFile(user, JSON.stringify(user2), function writeJSON(err) {
                    if (err) {
                        ...
                        return message.lineReply(embed20);
                    }
                });
            } else {
                const embed10 = new Discord.MessageEmbed()
                    .setTitle("Galvin")
                    .setDescription(`LOL imagine begging gtfo.`)
                    .setColor("RANDOM")
                    .setFooter(
                        `Imagine rejected while begging lol`,
                        message.author.displayAvatarURL({
        ...
        try {
            ...
            if (user2.money < 100) {
                return message.lineReply("You atleast need 100 gems?");
            }
            if (parseInt(amount) > parseInt(user2.money)) {
                return message.lineReply("you don't have enough money.");
            } else if (message.content.includes("-")) {
                ...
                return message.lineReply(`You need to bet something, use your brain cells.`);
            }
            user2.playid = 1;
            fs.writeFile(user, JSON.stringify(user2), function writeJSON(err) {
                if (err) console.log(err);
            ...
            const embed2 = new Discord.MessageEmbed()
                ...
                .setColor("#00FF00")
                .setTimestamp();
            let msg = await message.lineReply(embed2)
            await msg.react("👊")
            await msg.react("🛑")
            await msg.react("🚫")
            ...
            msg.awaitReactions(filter, {
                ...
                async (collected) => {
                    ...
                    if (reaction.emoji.name == "👊") {
                        ...
                        if (player1 > 21 && player2 > 21) {
                            ...
                            fs.writeFile(user, JSON.stringify(user2), function writeJSON(err) {
                                if (err) {
                                    return message.lineReply("Fucks da error");
                                } else {
                ...
                return message.lineReply(`:stopwatch: | **${message.author.username }** passed the appointed time **20** seconds, you immediately lost ⏣ **${args[0].toLocaleString()}**.`);
            })
        } catch (err) {
        ...
        try {
            ...
            if (chance2 > 100) {
                ...
                let result3 = new Discord.MessageEmbed()
                    ...
                    .setColor("#FF0000")
                    .setTimestamp();
                return message.lineReply(result3)
                limitation.add(message.author.id);
            } else if (chance2 < 100) {
                let result4 = new Discord.MessageEmbed()
                    ...
                    .setColor("#FF0000")
                    .setTimestamp();
                return message.lineReply(result4)
            } else {
                let result3 = new Discord.MessageEmbed()
                    .setTitle(`Top Rich users`)
        ...
        if (updaterich == true) {
            return message.lineReply(`:stopwatch: | **${message.author.username}** Please wait **5 minutes** to use this command.`);
        } else
            updaterich = true;
        message.lineReply(`${message.author.username}, thanks for updating.`);
        ...
        try {
            ...
            return message.lineReply(`Something went wrong pls contact Galvin#7778`);
        }
        setTimeout(() => {
            updaterich = false;
        }, 300000);
        ...
        if (!sniped) {
            return message.lineReply(`${message.author.username}, there is nothing to snipe.`);
        } else if (sniped.content.length > 55) {
            return message.lineReply(`Damn, message is too big for me :>`);
        } else {
            let result3 = new Discord.MessageEmbed()
        ...
        if (!url) {
            ...
            return message.lineReply(`Aye ${message.author.username}, nice try.`)
        } else {
            try {
                request.get(`https://image.thum.io/get/maxAge/12/width/700/${url}`, function(err, response, data) {
                    if (err) {
                        return message.lineReply(`Ohh, fcks something went wrong "${err}"`);
                        ...
                        return message.lineReply(avatarEmbed);
                    } else {
                        const avatarEmbed = new MessageEmbed()
                            .setDescription(`Screenshot of ${url}`)
                            .setColor("RANDOM")
        ...
        if (!list.length) {
            return message.lineReply(`No results found for **${args.join(' ')}**.`);
        }
        ...
        let world = args[0];
        if (!world)
            return message.lineReply(`Please input the world you want to fetch.`)
        try {
            request.get(`https://s3.amazonaws.com/world.growtopiagame.com/${world}.png`, function(err, response, data) {
                ...
                if (response.statusCode == 403) {
                    return message.lineReply(`Seems like the world unaccessible for request`)
                }
        ...
        try {
            request.get(`https://growtopiagame.com/detail`, function(err, response, data) {
                if (err) {
                    return message.lineReply(`Cannot access the website.`)
                }
                const online = JSON.parse(data);

                if (response.statusCode == 403) {
                    return message.lineReply("`403 forbidden`")
                }
                if (response.statusCode === 200)
                    message.lineReply("There are `" + online['online_user'] + "` players online right now!")
            })
        } catch (err) {}
        ...
        const port = args[1]
        if (!ip) {
            return message.lineReply(`You need to fetch **(IP)** something.`);
        } else if (!port) {
            return message.lineReply(`You need to input the **(port)**.`);
        } else if (isNaN(+port) == true) {
            message.lineReply(`Hey, **${port}** does not match you can only use **integer** for **port**`);
        } else {
            fetch(`http://anabelle.bot.nu/api/sampquery?ip=${ip}&port=${port}`, { //
                method: "GET"
            }).then(data => data.json()).then(data => {
                if (data.response['serverip'] == undefined) {
                    return message.lineReply(`Something went wrong, **(${ip}:${port})** does not **up** or **under maintenance**.`);
                } else {
                    const embed = new Discord.MessageEmbed()
...
`);
            ...
            try {
                fetch("http://api.popcat.xyz/instagram?user=" + name, { //
                    ...
                    if (data['username'] == undefined) {
                        return message.lineReply(`Cannot find any user with name **${name}**`);
                    } else {
        ...
        if (command.length > 5) {
            return message.lineReply(`What do you mean ${message.author.username}, type **".help"** to find out my commands.`);
        }
    }
